...
<table class="admin-table">
    <thead><tr><th>ID</th><th>User</th><th>Status</th><th>Roles</th><th>Last log on</th><th>Created</th><th>Actions</th></tr></thead>
    <tbody>{$rows}</tbody>
</table>
HTML;
    ...
    public function resendInvite(Request $request, TenantContext $tenant, ?array $currentUser): Response
    {
        ...
        $userId = (int) ($_POST['user_id'] ?? 0);
        $user = $userId > 0 ? $this->users->tenantUserById($tenant->tenantId, $userId) : null;
        if (!$user) {
        ...
        $this->queueInviteEmail($tenant, $email, $displayName);
        $this->auditLog?->record('tenant.user.invite_resent', $tenant->tenantId, (int) ($currentUser['user_id'] ?? 0), 'user', (string) $userId, ['email' => $email], $request->server('REMOTE_ADDR'));
