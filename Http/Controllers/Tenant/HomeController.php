...
HTML
    ...
    private function layout(TenantContext $tenant, string $title, string $body): string
    {
        ...
        $footerSignup = $this->footerSignupForm($tenant, $contactSlug);
...
<head>
    ...
    <script src="/assets/tenant-forms.js?v=20260601a" defer></script>
</head>
...
<footer class="site-footer">
    {$footerSignup}
    <div class="site-footer-copyright">© {$year} {$copyrightName}</div>
</footer>
</body>
</html>
HTML;
    ...
    /**
     * Renders the one public footer signup path. The retired delayed modal and
     * floating prompt are intentionally not emitted so visitors never see two
     * competing mailing-list interfaces on the same page.
     */
    private function footerSignupForm(TenantContext $tenant, string $contactSlug): string
    {
        ...
        $csrf = $this->csrf ? $this->escape($this->csrf->getOrCreate()) : '';
        $siteTitle = $this->escape($this->settings->get($tenant, 'site_title', $tenant->name));
        $returnTo = '/' . trim($contactSlug, '/') . '?signup_sent=1';
        $captcha = FirstPartyCaptcha::render('signup', (int) $tenant->tenantId);

        return <<<HTML
<form method="post" action="/signup" class="footer-signup" data-af-async-form data-af-result="footer-signup-result" data-af-busy-label="Joining..." data-af-busy-message="Joining the email list...">
    <input type="hidden" name="csrf_token" value="{$csrf}">
    <input type="hidden" name="source" value="tenant_footer">
    <input type="hidden" name="return_to" value="{$this->escape($returnTo)}">
    <label>Join {$siteTitle}'s email list
        <span class="footer-signup-row"><input type="email" name="email" autocomplete="email" placeholder="Email address" required><button type="submit">Subscribe</button></span>
    </label>
    {$captcha}
