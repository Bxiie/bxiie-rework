...
HTML,
    ...
    public function resendInvite(Request $request, ?array $currentUser): Response
    {
        ...
        $userId = (int) ($_POST['user_id'] ?? 0);
        $user = $userId > 0 ? $this->users->userById($userId) : null;
        if (!$user || !$this->users->userIsPlatformUser($userId)) {
            return Response::html('<h1>Invalid platform user invite resend request</h1>', 422);
        }

        $email = (string) $user['email'];
        $displayName = trim((string) ($user['display_name'] ?? '')) ?: null;
        $this->queuePlatformInviteEmail($email, $displayName);
        $this->auditLog?->record('platform.user.invite_resent', null, (int) ($currentUser['user_id'] ?? 0), 'user', (string) $userId, ['email' => $email], $request->server('REMOTE_ADDR'));
        FlashMessages::success('Platform user invite resent.');
    ...
    private function queuePlatformInviteEmail(string $email, ?string $displayName): void
    {
...
You have been invited to administer the ArtsFolio platform.
...
ArtsFolio";
        $bodyHtml = '<p>' . htmlspecialchars($nameLine, ENT_QUOTES, 'UTF-8') . '</p>'
            . '<p>You have been invited to administer the ArtsFolio platform.</p>'
    ...
    private function noticeHtml(string $notice): string
    {
        $message = match ($notice) {
            ...
            'invite-resent' => 'Platform user invite resent.',
            'status-updated' => 'Platform user status updated.',
            'suspend' => 'Platform user suspended.',
            'delete' => 'Platform user deleted.',
            default => '',
        };

        return $message !== '' ? '<p class="admin-notice admin-notice-success">' . AdminLayout::escape($message) . '</p>' : '';
    }
