...
final class SocialFrontController
{
    ...
    private function compose(TenantContext $tenant, ?array $currentUser): Response
    {
        ...
        $sharedCount = $this->social->publishedPostCountForArtwork($tenant->tenantId, $artworkId);
        $sharedNotice = $sharedCount > 0
            ? '<p class="admin-notice admin-notice-warning"><strong>Already shared on Instagram.</strong> This artwork appears in ' . $sharedCount . ' previously published Instagram post' . ($sharedCount === 1 ? '' : 's') . '. Publishing again will create another post.</p>'
            : '';
        $accountNotice = '';
        if ($existing) {
