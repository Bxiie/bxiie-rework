...
return static function (Router $router, array $context): void {
    ...
    $router->get('/platform/admin/operations/metrics', fn (Request $request): Response => (new PlatformAdminOperationsController(new RequirePlatformRole(new MembershipRepository($pdo)), new OperationsMonitorRepository($pdo), new CsrfTokenService(), new AuditLogRepository($pdo), new OperationsTaskLauncher()))->metric($request, $currentUser));
    $router->get('/platform/admin/operations/runs/{id}', fn (Request $request, array $params): Response => (new PlatformAdminOperationsController(new RequirePlatformRole(new MembershipRepository($pdo)), new OperationsMonitorRepository($pdo), new CsrfTokenService(), new AuditLogRepository($pdo), new OperationsTaskLauncher()))->run($request, $currentUser, (int) ($params['id'] ?? 0)));
    ...
    $router->get('/platform/admin/email-templates', fn (Request $request): Response => (new PlatformAdminEmailTemplatesController(new RequirePlatformRole(new MembershipRepository($pdo)), new CsrfTokenService(), new AuditLogRepository($pdo), new PlatformSettingsRepository($pdo), $root . '/template/email'))->index($request, $currentUser));
    $router->post('/platform/admin/email-templates/status', fn (Request $request): Response => (new PlatformAdminEmailTemplatesController(new RequirePlatformRole(new MembershipRepository($pdo)), new CsrfTokenService(), new AuditLogRepository($pdo), new PlatformSettingsRepository($pdo), $root . '/template/email'))->updateStatus($request, $currentUser));
    $router->post('/platform/admin/email-templates/schedule', fn (Request $request): Response => (new PlatformAdminEmailTemplatesController(new RequirePlatformRole(new MembershipRepository($pdo)), new CsrfTokenService(), new AuditLogRepository($pdo), new PlatformSettingsRepository($pdo), $root . '/template/email'))->updateSchedule($request, $currentUser));
    $router->post('/platform/admin/email-templates/create', fn (Request $request): Response => (new PlatformAdminEmailTemplatesController(new RequirePlatformRole(new MembershipRepository($pdo)), new CsrfTokenService(), new AuditLogRepository($pdo), new PlatformSettingsRepository($pdo), $root . '/template/email'))->create($request, $currentUser));
    $router->post('/platform/admin/email-templates/rename', fn (Request $request): Response => (new PlatformAdminEmailTemplatesController(new RequirePlatformRole(new MembershipRepository($pdo)), new CsrfTokenService(), new AuditLogRepository($pdo), new PlatformSettingsRepository($pdo), $root . '/template/email'))->rename($request, $currentUser));
    $router->post('/platform/admin/email-templates', fn (Request $request): Response => (new PlatformAdminEmailTemplatesController(new RequirePlatformRole(new MembershipRepository($pdo)), new CsrfTokenService(), new AuditLogRepository($pdo), new PlatformSettingsRepository($pdo), $root . '/template/email'))->update($request, $currentUser));
