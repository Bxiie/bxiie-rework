...
return static function (Router $router, array $context): void {
    ...
    $router->post('/admin/billing/free-access-code', fn (Request $request): Response => (new TenantAdminBillingController(new RequireTenantRoleBrowser(new MembershipRepository($pdo)), $pdo))->applyFreeAccessCode($request, $tenant, $currentUser));
        ...
        $router->get('/admin/sales', fn (Request $request): Response => (new TenantAdminSalesController(new RequireTenantRoleBrowser(new MembershipRepository($pdo)), new SalesRepository($pdo), new CsrfTokenService(), new AuditLogRepository($pdo), new PlatformSettingsRepository($pdo)))->index($request, $tenant, $currentUser));
        $router->post('/admin/sales/update', fn (Request $request): Response => (new TenantAdminSalesController(new RequireTenantRoleBrowser(new MembershipRepository($pdo)), new SalesRepository($pdo), new CsrfTokenService(), new AuditLogRepository($pdo), new PlatformSettingsRepository($pdo)))->update($request, $tenant, $currentUser));
        $router->post('/admin/sales/refund', fn (Request $request): Response => (new TenantAdminSalesController(new RequireTenantRoleBrowser(new MembershipRepository($pdo)), new SalesRepository($pdo), new CsrfTokenService(), new AuditLogRepository($pdo), new PlatformSettingsRepository($pdo)))->refund($request, $tenant, $currentUser));
