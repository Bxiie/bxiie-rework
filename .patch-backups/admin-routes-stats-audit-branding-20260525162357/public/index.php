...
// Tenant login is intentionally mounted at /login on each tenant domain; the tenant root remains public content.
    ...
    $router->get('/login', fn (Request $request): Response => (new LoginController(new PasswordAuthService(new UserRepository($pdo), new UserIdentityRepository($pdo), new PasswordHasher(), new SessionRepository($pdo), new SessionTokenService()), new CsrfTokenService()))->show($request));
        ...
        $router->get('/help', fn (Request $request): Response => (new MarketingController($pdo))->help($request));
    ...
    $router->get('/auth/google', fn (Request $request): Response => (new OAuthController())->start($request, 'google'));
    $router->get('/auth/google/callback', fn (Request $request): Response => (new OAuthController())->callback($request, 'google'));
    $router->get('/auth/facebook', fn (Request $request): Response => (new OAuthController())->start($request, 'facebook'));
    $router->get('/auth/facebook/callback', fn (Request $request): Response => (new OAuthController())->callback($request, 'facebook'));
