...
{
    public static function login(string $action = '/login', string $message = '', string $csrfToken = '', string $brandName = 'ArtsFolio', string $heading = 'Sign in to ArtsFolio'): string
    {
        ...
        $csrfField = $csrfToken !== '' ? '<input type="hidden" name="csrf_token" value="' . self::escape($csrfToken) . '">' : '';

        return self::page('Sign in', <<<HTML
<p class="auth-eyebrow">Welcome back</p>
<h1>{$safeHeading}</h1>
<p class="auth-copy">Manage {$safeBrand}: artwork, portfolio sections, events, messages, email signups, analytics, audit history, discovery settings, and public content.</p>
{$notice}
<div class="sso-row">
