...
## 2026-06-27 Platform dashboard paying metric emergency rollback

- Temporarily removed the new Actual paying tenants dashboard metric after /platform/admin returned HTTP 500.
- Tenant search changes are not rolled back by this script.
...
## Deleted tenant slug reuse

- Platform soft deletion changes the tenant slug to a deterministic internal tombstone slug.
- Public signup lazily repairs older deleted tenants that still hold their original slug.
- Only tenants with `status = deleted` release their slug; all other statuses continue to block reuse.
- The `tenants.slug` unique constraint remains the concurrency safeguard.
- No schema migration is required.
- Regression coverage: `scripts/test/deleted_tenant_slug_reuse_static.php`.


## Tenant signup slug availability regression test
...
## Signup post-registration email

- Successful tenant signup queues both email verification and a welcome message.
- The welcome message includes the public site URL and direct `/admin` URL.
- Duplicate pending verification and welcome messages are suppressed.
- Existing accounts can be repaired with `scripts/admin/queue_signup_post_registration_email.php`.
- Regression coverage: `scripts/test/signup_post_registration_email_static.php`.
...
## Queue deadlock recovery for sales.inventory.release_expired (2026-09-01)

- Root cause of platform job failure (job id 38564, `SQLSTATE[40001]: Deadlock found when trying to get lock`): `SalesRepository::releaseExpiredReservations()` and checkout's `expireReservationsWithinTransaction()` both range-scan the same `sales_inventory_reservations` `(status, expires_at)` index. When a customer's checkout ran concurrently with the recurring cleanup job, InnoDB gap-lock collisions between the two produced the deadlock.
- `app/Tenant/Sales/SalesRepository.php`: `releaseExpiredReservations()` now retries automatically on SQLSTATE 40001 via a new private `withDeadlockRetry()` helper (up to 3 attempts, short jittered backoff). Deadlocks on this path are transient and MariaDB's own error text recommends restarting the transaction.
- `scripts/workers/run_once.php`: the `analytics.rollup`, `sales.cart.queue_abandoned_reminders`, and `sales.inventory.release_expired` cases now re-enqueue their next cycle from a `finally` block instead of only after a successful `handle()`. Previously, a single failed run permanently stopped that recurring job (no future cycle was ever queued) until an admin manually clicked Requeue.
- Added `scripts/test/queue_deadlock_recovery_static.php`, wired into `scripts/test/preflight.sh` immediately after `background_job_concurrency_static.php`. Verified via `git stash` that it fails against the pre-fix source and passes after the fix.
- Known separate defect, not fixed in this change: the platform admin job detail page (`/platform/admin/jobs/{id}`, `JobsController::show()`) does not render Requeue/Cancel actions — those only exist on the job list view (`/platform/admin/jobs`). Use the list view, filtered by `job_type` or `status=failed`, to requeue or cancel a job until the detail page is fixed.
- No schema migration. No service restart required: `scripts/workers/run_forever.php` spawns a fresh `run_once.php` process via `passthru()` each cycle, so the fix takes effect on the next queue cycle after deploy.

<!-- End of file. -->

## Job detail action buttons, email-signup spam probability, and AJAX delete (2026-09-01)

- **Platform job detail page actions**: `JobsController::show()` (`/platform/admin/jobs/{id}`) previously rendered no Requeue/Cancel controls at all — only the list view (`/platform/admin/jobs`) had them. Added the same two forms to the detail page, plus a `return_to` field (validated by a new `JobsController::returnTo()` helper, matching the pattern already used by `EmailSignupsController`/`ContactMessagesController`) so acting from the detail page redirects back to that job instead of always bouncing to the list.
- **Email-signup spam probability** (`app/Tenant/Signup/SpamScoreService.php`, new): a deterministic heuristic scorer (disposable-domain denylist, suspicious TLDs, role-address local parts, auto-generated-looking local parts, missing name+source, same-IP signup velocity in the last 60 minutes) — no outbound network calls, no third-party API, so it adds no latency or new failure mode to the public signup form. Wired into both creation paths: `EmailSignupService::receive()` (public `/signup`) and `EmailSignupsController::import()` (tenant admin CSV import). Score is only computed for genuinely new addresses; `EmailSignupRepository::upsert()` stores it in the new `email_signups.spam_probability` column (migration `0069_email_signup_spam_probability.sql`, nullable, plus a supporting `(tenant_id, ip_address, created_at)` index for the velocity check) and never overwrites it on a repeat signup. Displayed as a color-coded percentage on the tenant admin email-signups list (`/admin/email-signups`); existing pre-migration rows show "Not scored" rather than a misleading 0%.
- **Email-signup delete without a page reload**: the Delete control on `/admin/email-signups` is now a `fetch()`-driven button (event-delegated script rendered once per page, not per row) instead of a POST `<form>`. `EmailSignupsController::delete()` detects the `X-Requested-With: XMLHttpRequest` header and returns `{"ok":true}` JSON instead of a redirect in that case; the client removes just the deleted row's `<tr>` from the DOM, so an operator can delete several signups in a row without losing scroll position. Confirm/Unsubscribe are unchanged (still normal reloading form posts) since only delete was requested.
- Added `scripts/test/job_detail_actions_static.php`, `scripts/test/email_signup_spam_probability_static.php`, and `scripts/test/email_signup_ajax_delete_static.php`, all wired into `scripts/test/preflight.sh`. Each was verified via `git stash`/temporary file removal to fail against the pre-fix source.
- No service restart required for the job-detail or delete changes (plain code). The spam-probability migration should be applied via the normal migration runner before deploying the code that references `spam_probability`/the new index.
- Not run: full DB-backed `scripts/test/preflight.sh` and the pre-existing `scripts/test/email_signup_consent.php`/`tenant_admin_lists.php` integration tests — local Docker daemon is not running, so the dev MariaDB container is unreachable in this environment. The new `?int $spamProbability = null` / `?SpamScoreService $spam = null` constructor and method params are appended at the end and are all-optional, and every existing call site already uses named arguments, so this is backward compatible by construction; confirm with a real run once Docker/DB is available.
